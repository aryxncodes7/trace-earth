...
export default function Navbar({ activeTab, setActiveTab, email, user = null, onLogout }: NavbarProps) {
  ...
  useEffect(() => {
    ...
    const handleOutsideClick = (e: MouseEvent) => {
      ...
      if (container && !container.contains(e.target as Node)) setDropdownOpen(false);
    };
  ...
  return (
    <>
      <header
        id="desktop-navbar"
        role="banner"
        className="sticky top-0 z-40 w-full border-b border-slate-200 bg-white/95 backdrop-blur-sm shadow-sm dark:border-zinc-800 dark:bg-gray-900/95"
      >
        <div className="mx-auto flex h-14 max-w-7xl items-center justify-between px-6">
          <div
            ...
            role="link"
            tabIndex={0}
            aria-label="trace.earth home"
            onKeyDown={(e) => e.key === 'Enter' && setActiveTab('dashboard')}
          >
            <svg width="140" height="32" viewBox="0 0 140 32" xmlns="http://www.w3.org/2000/svg" className="h-8 w-auto" aria-label="trace.earth logo" role="img">
              <title>trace.earth</title>
          ...
          <nav className="hidden md:flex items-center space-x-1" role="navigation" aria-label="Main navigation">
            {tabs.map((tab) => {
              ...
              if (activeTab === 'landing' && tab.id !== 'profile') return null;
              return (
                <button
                  key={tab.id}
            ...
            {activeTab === 'landing' && (
              <button
                ...
                aria-label="Log in to trace.earth"
          ...
          <div className="flex items-center space-x-3">
            ...
            >
              {isDark
                ...
                : <Moon className="h-3.5 w-3.5 text-slate-600" aria-hidden="true" />
              }
            </button>

            {email && activeTab !== 'landing' && (
              <div className="relative flex items-center border-l border-slate-200 pl-3 dark:border-zinc-800" id="avatar-dropdown-container">
                <button
                  type="button"
                ...
                >
                  {isImageUrl ? (
                    <img src={user.image || ''} alt={`${user?.name || 'User'} profile picture`} referrerPolicy="no-referrer" className="h-full w-full rounded-full object-cover" />
                  ) : user?.image ? (
                    <span className="text-base leading-none select-none" aria-hidden="true">{user.image}</span>
                  ) : (
                    <span className="font-sans text-xs font-semibold text-slate-700 dark:text-zinc-200 uppercase select-none" aria-hidden="true">
                ...
                {dropdownOpen && (
                  <div
                    className="absolute right-0 top-full mt-2 w-48 rounded-lg border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 py-1.5 shadow-lg ring-1 ring-black/5 z-50"
                    role="menu"
                  ...
                  >
                    ...
                    >
                      <Settings className="mr-2 h-3.5 w-3.5 text-slate-400 dark:text-zinc-500" aria-hidden="true" />
                    ...
                    >
                      <LogOut className="mr-2 h-3.5 w-3.5 text-red-500 dark:text-red-400" aria-hidden="true" />
                      <span>Log Out</span>
                    </button>
                  </div>
                )}
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                aria-label={tab.label}
                aria-current={isActive ? 'page' : undefined}
                className="flex flex-col items-center justify-center w-14 h-12 py-1 select-none cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-green-500 rounded-lg"
                style={{ minHeight: '44px', minWidth: '44px' }}
              >
                <Icon className={`h-4.5 w-4.5 transition-colors ${isActive ? 'text-green-600' : 'text-slate-400 dark:text-zinc-400'}`} aria-hidden="true" />
                <span className={`mt-1 font-sans text-[10px] font-medium tracking-tight transition-colors ${isActive ? 'text-green-600' : 'text-slate-400 dark:text-zinc-400'}`}>
                  {tab.label.split(' ')[0]}
                </span>
              </button>
            );
