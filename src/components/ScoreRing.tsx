...
interface ScoreRingProps {
  score: number; // Today's footprint total in kg
  target: number; // User personal goal (e.g. 10.0)
}

export default function ScoreRing({ score, target }: ScoreRingProps) {
  // Max score for SVG mapping representation (e.g., 20kg is 100%)
  const maxCap = 20.0;
  const radius = 80;
  ...
  // Determine indicator colors based on exact threshold specs
  // Green (< 8kg) | Amber (8kg - 13kg) | Red (> 13kg)
  let statusColor = 'text-green-600';
  let strokeColor = '#16a34a'; // bg-green-600
  let ringBgColor = 'text-green-50';
  let statusLabel = 'Pretty good';
  let statusDesc = 'Below target levels - exceptional climate control!';

  if (score >= 8.0 && score <= 13.0) {
    statusColor = 'text-amber-500';
    strokeColor = '#f59e0b'; // bg-amber-500
    ringBgColor = 'text-amber-50';
    statusLabel = 'Moderate footprint';
    statusDesc = 'Close to benchmarks. Look for small optimization areas.';
  } else if (score > 13.0) {
    statusColor = 'text-red-500';
    strokeColor = '#ef4444'; // bg-red-500
    ringBgColor = 'text-red-50';
      ...
      <div className="relative flex items-center justify-center">
        ...
        <svg className="h-52 w-52 transform -rotate-90">
          ...
          <circle
            ...
            fill="transparent"
          />
          {/* Active colored performance stroke */}
          <motion.circle
            cx="104"
            cy="104"
            r={radius}
       ...
       {/* Mini Rating Label and Goal Delta */}
       <div className="mt-4 text-center">
         <span className={`inline-flex px-2.5 py-0.5 rounded-full text-xs font-normal tracking-wide bg-slate-50 border border-slate-100 dark:bg-zinc-900 dark:border-zinc-800 ${statusColor}`}>
           {statusLabel}
         </span>
